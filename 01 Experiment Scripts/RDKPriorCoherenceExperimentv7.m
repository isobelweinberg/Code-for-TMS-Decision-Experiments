%% Moving RDK - 0% coherence
  try
    ...
    NumDots=150;
    CoherenceArray=[10 50 100];%array of coherences you want to use, as a percentage
    LeftProbability=[10 5 100]; %probability the RDK goes left, as a percentage
    TrialsPerBlock=5; 
    ...
    NumBlocks = TotalNumTrials/TrialsPerBlock;
    
    %Human error check
    if numel(LeftProbability)~=NumBlocks;
...
%% Collect the results
   ...
   if round(PositionInBlock)==PositionInBlock 
       Screen('DrawText', windowNo, 'Have a rest. Press any key to continue.', xmiddle, ymiddle, [0 0 0]);
       Screen('Flip', windowNo);
       KbStrokeWait;
   end
