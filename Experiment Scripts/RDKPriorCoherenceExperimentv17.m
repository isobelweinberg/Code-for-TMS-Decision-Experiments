%% Moving RDK - 0% coherence
  try
    ...
    LeftProbabilityArray=[100 100]; %probability the RDK goes left, as a percentage
    TrialsPerBlock=5; 
    ...
    ReactionTime=zeros(1,TotalNumTrials);
 ...
 %% Present the Stimuli
   ...
   %time=0;
   StimulusOnset=0; %used in the RT calculation
   ...
   while KeyPress==0
       ...
       [Timestamp, FrameOnsetTime]=Screen('Flip', windowNo, Timestamp+(0.5*FlipInterval)); 
       %time=time+ifi;
       
       if latestdotpositions==dotpositions %if this is the first frame
           StimulusOnset=FrameOnsetTime; %used in RT calculation
       end     
...
%% Collect the results
    ReactionTime(1,TrialNo)=(KeyPressTime-StimulusOnset)*1000;%in ms; IS THIS RIGHT?? maybe make it flip start...
