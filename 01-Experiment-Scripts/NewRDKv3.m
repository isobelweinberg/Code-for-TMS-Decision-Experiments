try
    ...
    for TrialNo = 1:TotalNumTrials %find out how many nots need to be coherent on each trial
        ...
        for DotNumber = 1:data.NumCoherentDots(1,TrialNo) % allocate coherent positions to a subset
            %for coherent dots, after frame 1, their angle is always going
            %to be 0 or 180
            if data.Direction(1, TrialNo) == -1 %if left
                Angles(:, DotNumber, 2:TotalNumFrames, TrialNo) = 180; %generate some random angles
            elseif data.Direction(1, TrialNo) == 1 
                Angles(:, DotNumber, 2:TotalNumFrames, TrialNo) = 0;
            end
            
       %coherent dots - their radius increases by a fixed amount
       %(speedxtime) on each trial
       for FrameNo = 2:TotalNumFrames
        Radii(:, DotNumber, FrameNo, TrialNo) = Radii(:, DotNumber, FrameNo-1, TrialNo) + (data.Direction(1,TrialNo)*DotSpeed*IFI);
        %stop dots going off the end
       if Radii(:, DotNumber, FrameNo, TrialNo) > (ApertureRadius-DotRadius)
           Radii(:, DotNumber, FrameNo, TrialNo) = -(mod(Radii(:, DotNumber, FrameNo, TrialNo), ApertureRadius))
