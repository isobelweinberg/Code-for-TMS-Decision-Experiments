...
for BlockNo=1:TotalNumBlocks
    ...
    data.LeftProbability(1, BlockNo) = LeftProbabilityArray(1, (data.ProbabilityOrder(1, BlockNo)));
    ...
    for TrialNo =(((BlockNo-1)*TrialsPerBlock)+1):(BlockNo*TrialsPerBlock)
        ...
        if option.TMS == 1 && randi(100) <= TMS.Probability %find out if this will be a TMS trial
            data.TMSindex(1, TrialNo) = randi(numel(TMS.Timepoints));
             data.TMSTimepoint(1, TrialNo) = TMS.Timepoints(1, (data.TMSindex(1,TrialNo))); %timepoint
             data.TMSRelation(1, TrialNo) = TMS.TimepointRelations(1, (data.TMSindex(1,TrialNo))); %relation
