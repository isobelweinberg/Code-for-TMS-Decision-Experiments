%% Moving RDK - 0% coherence
  try
    ...
    %% ParticipantDetails
    Name='Isobel_Weinberg'; %use underscores
    Age=25;
    ...
    LeftProbabilityArray=[5]; %probability the RDK goes left, as a percentage
    TrialsPerBlock=10; 
    ...
    % Send 3 and then 0 to parallel port???
    if TMS
        outportb(888, 3); 
        wait(2); 
        outportb(888, 0); 
...
%% Collect the results
    ReactionTime(1,TrialNo)=(KeyPressTime-StimulusOnset)*1000;%in ms; maybe make it flip start...
