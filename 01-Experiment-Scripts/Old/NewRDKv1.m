try
    ...
    %% Inputs
    DotRadius = 10; %pixels
    CoherenceArray = [5 20 90]; %percent
...
%     rand('seed', sum(100 * clock)); %legacy
    ...
    %% Open a window
    windowNo = Screen('OpenWindow', 0, BackgroundColour);%0 is the main screen
...
%                end
               ...
               DotsXY(1, DotNumber, FrameNo, TrialNo) = mod((DotsXY(1, DotNumber, FrameNo, TrialNo))+ApertureRadius, ApertureRadius*cosd(Angles(1, DotNumber, FrameNo, TrialNo)))-ApertureRadius;
   ...
   % left is to draw them!
    ...
    for TrialNo=1:TotalNumTrials
        ...
        Screen('DrawDots', windowNo, FixationXY, 2*FixationRadius, DotColour);
        ...
        for FrameNo=1:TotalNumFrames
            Screen('DrawDots', windowNo, DotsXY(:, :, FrameNo, TrialNo), DotColour);
