function [FixationXY, DotsXY, data] = generate_stimuli(NumDots, TotalNumTrials, TrialsPerBlock, TotalNumBlocks, vars, params, participant, option)
...
DotsXY = zeros(2, NumDots, params.TotalNumFrames, TotalNumTrials); %preallocate
...
Radii = sqrt(rand(1, NumDots, params.TotalNumFrames, TotalNumTrials))*screen.ApertureRadius; %generate some random distances from the centre of the aperture 
%sqrt is needed to stop dots clustering in middle
Angles = rand(1, NumDots, params.TotalNumFrames, TotalNumTrials)*360; %generate some random angles
DotsXY(1, :, :, :) = Radii.*cosd(Angles);% fill with random X coordinates
DotsXY(2, :, :, :) = Radii.*sind(Angles); % fill random Ys

%Used to order the probabilities
data.ProbabilityOrder = randperm(TotalNumBlocks);

for BlockNo=1:TotalNumBlocks
    % Set Probability the dots will go left - determined by block
    data.LeftProbability(1, BlockNo) = vars.LeftProbabilityArray(1, (data.ProbabilityOrder(1, BlockNo)));
    ...
    for TrialNo =(((BlockNo-1)*TrialsPerBlock)+1):(BlockNo*TrialsPerBlock)
        ...
        if exist('participant.threshold', 'var')
            data.Coherence(1, TrialNo) = participant.threshold*100*vars.CoherenceArray(1,(randi(numel(vars.CoherenceArray))));
        else
            data.Coherence(1, TrialNo) = vars.CoherenceArray(1,(randi(numel(vars.CoherenceArray)))); %pick a coherence at random from the Coherence Array for this trial
        end
        ...
        if option.TMS == 1 && randi(100) <= vars.TMS.Probability %find out if this will be a TMS trial
            data.TMSindex(1, TrialNo) = randi(numel(vars.TMS.Timepoints));
             data.TMSTimepoint(1, TrialNo) = vars.TMS.Timepoints(1, (data.TMSindex(1,TrialNo))); %timepoint
             data.TMSRelation(1, TrialNo) = vars.TMS.TimepointRelations(1, (data.TMSindex(1,TrialNo))); %relation
...
%                  data.TMSTimepoint(1, TrialNo) = TMS.StimTimepoints(1,TMSindex);      
        ...
        for DotNumber = 1:data.NumCoherentDots(1,TrialNo) % allocate coherent positions to a subset
            ...
            DotsXY(2, DotNumber, 2:params.TotalNumFrames, TrialNo) = DotsXY(2, DotNumber, 1, TrialNo);
            %X coord incremements by a fixed amount (speed x time)
            for FrameNo = 2:params.TotalNumFrames
                DotsXY(1, DotNumber, FrameNo, TrialNo) = DotsXY(1, DotNumber, FrameNo-1, TrialNo) + (data.Direction(1,TrialNo)*screen.DotSpeed*params.IFI);
                %is the dot outside the circle?
                y = DotsXY(2, DotNumber, FrameNo, TrialNo);
                Width = sqrt(((screen.ApertureRadius)^2)-((y)^2));
                %Width = CircleWidth((DotsXY(2, DotNumber, FrameNo, TrialNo)), screen.ApertureRadius); give the function the Y and the circle radius and it gives you the X coord of the circle at that point
                if abs(DotsXY(1, DotNumber, FrameNo, TrialNo)) + screen.DotRadius > Width %if outside circle
