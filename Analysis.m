...
RTsbyLProbability=cell(3,TotalNumBlocks); %create empty variable
...
RTsbyCoherence=cell(3,numel(CoherenceArray)); %create emptyvariable

j=CoherenceArray(1,1);

for j=CoherenceArray(1:end)
    %make the coherence array the TOP ROW of RTsbyCoherence
    RTsbyCoherence{1,j} = j;
    
    %take all the elements of Reaction Time where Coherence==j
    %assign this to the SECOND ROW of RTsbyLProbability
    RTsbyCoherence{2,j} = results.ReactionTime(results.Coherence == j);
    ...
    %nb direction here refers to the direction of the stimulus, not the
    %response
    RTsbyCoherence{4,j} = results.ReactionTime(results.Direction == 1 & results.Coherence == j); %left
    RTsbyCoherence{6,j} = results.ReactionTime(results.Direction == 2 & results.Coherence == j); %right
